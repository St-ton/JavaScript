...
/* С помощью объекта location мы можем получить текущий URL или перенаправить браузер по новому адресу. */
...
// todo Первый и последний дочерние элементы
...
//todo Коллекция childNodes содержит список всех детей,

//включая текстовые узлы.
const childNodes2 = bodyElement.childNodes;
console.log(childNodes2);

//todo Для проверки наличия дочерних узлов существует также специальная функция hasChildNodes()
...
//todo Соседние и родительский узлы
...
/* Навигационные свойства, описанные выше, относятся ко всем узлам в документе. Так в childNodes находятся и текстовые узлы и узлы-элементы и узлы-комментарии, если они есть.*/
...
// todo Получаем коллекцию всех дочерних элементов
...
// todo Соседние и родительский элементы
...
//todo Поиск произвольного элемента
...
/* В данном примере, когда мы выбрали ".lesson__list .lesson__item-sub-list" в контексте <ul> с классом "lesson__sub-list", элемент с классом ".lesson__item-sub-list" были всё равно найден, хотя .lesson__list не является потомком элемента в котором происходил поиск (".lesson__sub-list").
...
/* 1. id - уникален 2. Поиск полько внутри document.*/
...
//todo document.getElementById(ID) и elem.getElementsBy;
/* На данный момент, они используются редко, так как querySelectorAll может их заменить. Но отличия все же есть! */
...
//todo Живая статическа коллекции
...
const lessonList = document.querySelector('.lesson__list');
lessonList.insertAdjacentHTML('beforeend', '<li class="lesson__item-list">Новый пункт</li>');
...
// todo Cодержимое элемента innerHTML
...
//todo  Cодержимое элемента целиком outerHTML
...
//todo Просто текст элемента textContent
...
// todo  Cодержимое текстового узла/комментария data.
...
// todo Создание нового элемента (тега) createElement
...
// todo Создание нового текстового узла createTextNode
...
// todo Клонирование узлов cloneNode - tсли нужен не перенос а именно копия элемента
...
//todo Вычисленные стили. getComputedStyle(element, [pseudo])
...
/* Чтобы получить конкретное значение Следует писать точное (полное) свойство*/
...
/* Мы использовали произвольные атрибуты, но это рискованно.
