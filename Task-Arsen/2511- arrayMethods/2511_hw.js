...
//! 1. Написать функцию, которая принимает имя и фамилию пользователя и возвращает true, если пользователь с таким именем делал заказ и  false, если пользователь с таким именем ничего не заказывал

console.log(' ***** 1 *****');
let isUserDidOrder = function (uName, uLastName) {
  let idByName = customers.find(
    ({ firstName, lastName }) => firstName == uName && lastName == uLastName
  ).id;
...
//! Написать функцию, которая принимает имя и фамилию и возвращает массив продуктов, которые заказал этот пользователь. У одного пользователя может быть несколько заказов. Итоговый массив должен быть одномерным. То есть должен содержать только объекты продуктов

console.log(' ***** 2 *****');

let userOrdersList = function (name, surname) {
  let getUserId = customers.find(
...
let notOrderedUsers = function (customersTable, ordersTable) {
  return customersTable.filter(({ id }) => ordersTable.every(({ customerId }) => customerId != id));
};
